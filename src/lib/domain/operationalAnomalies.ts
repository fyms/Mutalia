...
export interface OperationalAnomaly {
  id:string; prestationId:string; dossierId:string; householdId:string; adherentName:string;
...
export function describeControl(message:string) {
  if(message === DATA_TO_VERIFY) return {code:"contract",type:"Donnée contractuelle à vérifier",cause:DATA_TO_VERIFY,impact:"Calcul et validation impossibles sans garantie contrôlée.",recommendation:"Compléter et contrôler la garantie, les droits et la référence documentaire dans la prestation."};
