...
export function Product({ reference, simulator=false, categoryFilter=false }: {reference:string;simulator?:boolean;categoryFilter?:boolean}) {
  ...
  return <>
    ...
    <Card><CardHeader title={`Garanties particuliers — ${reference}`} subtitle="Les candidats sont consultables uniquement ; leurs valeurs ne sont pas proposées au calcul."/>
      ...
      <div className="overflow-x-auto"><table className="w-full text-left text-xs"><thead><tr><th>Catégorie / prestation</th><th>Valeur / mode du document</th><th>Statut</th><th>Provenance</th></tr></thead><tbody>{rows.filter(g=>!category||g.category===category).map(g=><tr key={g.id} className="border-t border-border"><td className="p-2">{g.category}<br/>{g.benefit}</td><td className="p-2">{g.status==='verified'?<>{g.value} {g.unit} · Forfait<br/>{g.limit}<br/>{g.condition}</>:g.documentValue?<>{g.documentValue}<p className="m-help mt-1">Source PDF — non validée pour calcul</p></>:'Donnée 2026 à vérifier'}</td><td>{g.status==='verified'?<div className="space-y-1"><Badge tone="success">Vérifié</Badge>{!hasDeterminedApplication(g)&&<DataToVerifyBadge label={CONDITION_TO_VERIFY}/>}</div>:<DataToVerifyBadge/>}</td><td className="p-2">{g.sourceFile}<br/>Page {g.sourcePage}</td></tr>)}</tbody></table></div>
    </Card>
  </>;
}
