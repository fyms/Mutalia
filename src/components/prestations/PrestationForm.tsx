...
export function PrestationForm({households, initial, onDone}: {households:PrestationHouseholdChoice[]; initial?:Prestation; onDone?:()=>void}) {
  ...
  return <form className="m-panel space-y-3" onSubmit={event => {
    ...
    startTransition(async () => {
      const result = await savePrestationAction({householdId,memberId,
    ...
    <fieldset disabled={pending} className="grid gap-3 md:grid-cols-3"><legend className="sr-only">Données de la prestation</legend>
      ...
      <label><span className="m-label">Date de soins</span><input className="m-field" type="date" name="careDate" required defaultValue={initial?.careDate} /></label>
      <label><span className="m-label">Montant facturé (€)</span><input className="m-field" type="number" name="billed" min="0" step="0.01" required defaultValue={initial?.billed} /></label>
    ...
    <button className="m-button" disabled={pending}>{pending ? "Enregistrement…" : initial ? "Enregistrer la correction" : "Recevoir la prestation"}</button>{" "}
