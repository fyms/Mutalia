...
export function PostalCityFields({postalCode = "", city = ""}: {postalCode?: string; city?: string}) {
  ...
  return <>
    <label className="block"><span className="m-label">Code postal</span>
      <input name="postalCode" className="m-field" value={postal} required pattern="[0-9]{5}" maxLength={5} inputMode="numeric" onChange={e => {
    ...
    <div><label className="block"><span className="m-label">Ville</span>
      {manual ? <input name="city" className="m-field" required maxLength={100} value={selected} onChange={e => setSelected(e.target.value)}/> :
        <select name="city" className="m-field" required value={selected} disabled={!valid || status === "loading"} onChange={e => setSelected(e.target.value)}>
