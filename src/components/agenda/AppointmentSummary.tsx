...
export function AppointmentSummary({owner,householdId,history=false}:{owner:string;householdId?:string;history?:boolean}){
 ...
 <ul>{rows.map(p=><li key={p.id} className="py-2"><Link className="underline" href="/agenda">{p.date} · {p.startTime}–{endTime(p.startTime,p.durationMinutes)}</Link> · <Link className="text-brand underline" href={p.contactType==="prospect"?`/prospects#${p.prospectId}`:`/adherents/${p.householdId}`}>{p.adherentName}</Link>{p.contactType==="prospect"&&<span> · Prospect</span>} · {p.type} · {p.reason} · {p.status}</li>)}</ul>{!rows.length&&<p>Aucun rendez-vous {history?"passé":"à venir"}.</p>}</section>;
}
